...
class ConditionalWait:
    @inject
    def __init__(self, web_driver: WD, settings_file: BaseSettingsFile) -> None:
        self.__web_driver = web_driver
        self.__settings_data = settings_file
        self._timeout = settings_file.get_value('timeouts.timeoutCondition')
    ...
    def wait_for_exp_condition(self, timeout, locator, polling_interval=None, message='',
        ...
        self.__web_driver.implicitly_wait(0)
        ...
        ex_message = self.__resolve_message(message)
        wait = WebDriverWait(driver=self.__web_driver, timeout=timeout_in_seconds,
        ...
        try:
            ...
            self.__web_driver.implicitly_wait(self.__settings_data['timeouts']['timeoutImplicit'])
    ...
    def __resolve_conditional_timeout(self, timeout=None):
        if timeout is None:
            return self.__settings_data['timeouts']['timeoutCondition"']
        return timeout

    def __resolve_polling_interval(self, polling_interval=None):
        if polling_interval is None:
            return self.__settings_data['timeouts']['timeoutPollingInterval']
